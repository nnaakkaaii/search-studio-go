~/go/bin/xo query 'postgresql://root:postgres@127.0.0.1:5432/studio?sslmode=disable' -M -B -T Studio -o models/ << ENDSQL
SELECT
    s.studio_id,
    s.studio_name,
    s.introduction,
...
WHERE
    s.studio_id = %%studioID int%%
AND s.is_deleted IS FALSE
;
ENDSQL

~/go/bin/xo query 'postgresql://root:postgres@127.0.0.1:5432/studio?sslmode=disable' -M -B -T Studios -o models/ <<ENDSQL
SELECT
    s.studio_id,
    s.studio_name,
    s.introduction,
